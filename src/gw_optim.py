...
def gromov_iter_plot(t, C1, C2, G, save_path=None):
...
def compute_gromov_wasserstein(C1, C2, p, q, tol, gpu, ot_warm, reg, maxiter, loss_fun='square_loss'):
    assert(len(C1) == len(p))
    assert(len(C2) == len(q))
        ...
        if not gpu:
            if ot_warm and it > 0:
                T, log = sinkhorn_knopp(
                    p, q, tens, reg, init_u=log['u'], init_v=log['v'], log=True)
            elif ot_warm:
            ...
            if ot_warm and it > 0:
                T, log = bregman.sinkhorn(
                    p, q, tens, reg, returnAsGPU=True, log=True)
            elif ot_warm:
                T, log = bregman.sinkhorn(p, q, tens, reg, log=True)
            else:
                T = bregman.sinkhorn(
                    p, q, tens, reg, returnAsGPU=True)
...
def get_entropic_gromov_barycenters(mle, test_func, lang_mapping_fun, ls, N, Cs, ps, p, lambdas, entreg,
                                max_iter, gw_max_iter, mapping_entropy, tol=1e-9, verbose=False, log=False, init_C=None, loss_fun='square_loss'):
    """
    ...
    if init_C is None:
        xalea = np.random.randn(len(N), 300)
        xalea = mle.normalize_embeddings(xalea)
        C = mle._intra_language_distances(xalea)
        #C = Cs[0].copy()
    else:
        print("training from previous C matrix")
        C = init_C.copy()
    ...
    print("max_iter is"+str(max_iter))
    ...
    while(err > tol and cpt < max_iter):
        ...
        for i in range(len(ls)):
            for j in range(len(ls)):
                if i!=j:
                    mapping = compute_gromov_wasserstein(Cs[i], C, ot.unif(len(Cs[i])), ot.unif(len(C)), 1e-8, False, True, mapping_entropy, 50)
                    mapping2 = compute_gromov_wasserstein(C, Cs[j], ot.unif(len(C)), ot.unif(len(Cs[j])), 1e-8, False, True, mapping_entropy, 50)
        ...
        T = [ compute_gromov_wasserstein(Cs[s], C, ot.unif(len(C)), ps[s], tol, False, True, entreg, gw_max_iter, loss_fun) for s in range(S)]
...
class gromov_wass_solver():
    ...
    def solve(self, xs, xt, p, q, maxiter=200, plot_every=20, print_every=1, verbose=True, save_plots=None):
        ...
        **** Ported from POT package, modified to plot plans and compute downstream
        ...
        T = compute_gromov_wasserstein(C1, C2, p, q, self.tol, self.gpu, self.ot_warm, self.reg, maxiter, loss_fun='square_loss')
        return T
