        ...
        with open(outf, 'w') as f:
            ...
            for src_idx,tgt_idx in gold_dict.items():
                if src_idx > n or np.all([e>m for e in tgt_idx]):
                    ...
                    row = ""
                    if not all(elem in predicted for elem in targets):
                        row = "**"
                    row += '\t'.join([src_word,'||'] + predicted + [">>"] + targets)
                    f.write(row + '\n')
    ...
    def score_translations(self, gold_dict, verbose = False):
        ...
        for src_idx,tgt_idx in gold_dict.items():
            if src_idx > n or np.all([e>m for e in tgt_idx]):
                ...
                map_total_num += len(targets)
                for t in targets:
                    if t in topk_predictions[src_word]:
                        map_precision += 1./(topk_predictions[src_word].index(t)+1)
        ...
        precisions['map'] = map_precision / map_total_num
